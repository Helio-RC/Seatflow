...
> **向后兼容**：旧版单文件 `plugin.manifest.json` 格式（v1.0）仍受支持，系统自动转换为虚拟包（1 包 = 1 策略）。新插件应使用双层清单格式。`.apairplugin` 和 `.ap-plugin` 两种扩展名均可加载。
...
> **单策略包**（旧格式兼容）：只需项目根目录一个 `plugin.manifest.json`，系统自动转换为虚拟包。字段定义见下方「旧版 plugin.manifest.json 规范（兼容保留）」。

### 4. 构建并部署

```bash
dotnet build -c Release
# 将输出文件复制到 A_Pair 的插件目录：
...
v1.1+ 采用 **双层清单架构**（ADR-007）：包级 `plugins-manifest.json` 负责加载指令，策略级 `manifest.json` 负责元数据与声明式配置。旧版单文件 `plugin.manifest.json` 仍受兼容支持。
...
## 旧版 plugin.manifest.json 规范（兼容保留）

> v1.0 单文件格式，新插件建议使用上方双层清单。
...
> **单策略包**（旧格式兼容）：根目录直接放 `plugin.manifest.json` + DLL/Lua/CSX，系统自动转为虚拟包。
...
将 `.ap-plugin`（或旧版 `.apairplugin`）拖放到 A_Pair 的"插件管理"页面即可安装，系统自动解包并加载。
...
通用保障：无效清单静默跳过，不含有效清单文件（`plugins-manifest.json`、`manifest.json` 或旧版 `plugin.manifest.json`）的目录被忽略。
...
| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| 插件未出现在列表中 | 清单缺失或无效 | 检查 `plugins-manifest.json`（或旧版 `plugin.manifest.json`）是否存在且为有效 JSON |
