...
public class AssignSeatCommandTests
{
    ...
    public async Task ExecuteAsync_ValidAssignment_ShouldSucceed ()
    {
        ...
        var ws = new SeatingWorkspace(new[] { student } , new Seat[] { seat });

        var cmd = new AssignSeatCommand("seat1" , "s1");
        var result = await cmd.ExecuteAsync(ws , CancellationToken.None);
    ...
    public async Task ExecuteAsync_DuplicateStudent_ShouldFail ()
    {
        ...
        var ws = new SeatingWorkspace(new[] { student } , new Seat[] { seat1 , seat2 });
        // 预先分配 seat1 给 s1
    ...
    public async Task UndoAsync_ShouldClearOccupant ()
    {
        ...
        var cmd = new AssignSeatCommand("seat1" , "s1");
        await cmd.ExecuteAsync(ws);
        var result = await cmd.UndoAsync(ws);
