...
Conflict resolution = Priority number (first-come-first-served). Dependent strategies have their own internal priority ordering within RandomFill's context (DeskMate 50 → NoRepeatDeskMate 40). Handled assignments still run remaining dependents for inspection/warnings. See `docs/adr/ADR-006.md`.
