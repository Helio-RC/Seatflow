...
function Publish-One($SC,$Label,$Rids){
    ...
    foreach($rid in $Rids){
        ...
        if($LASTEXITCODE){Step "编译失败" Red;exit 1}
        $exe="$AppName$sf"
...
if($Mode-or$HashOnly){
    ...
    $sw.Stop();Write-Host "`n完成 $([math]::Round($sw.Elapsed.TotalSeconds,1))s" -F Cyan;ShaTable;exit 0
}
...
$sp=@($P|? Sel)
if(!$sp){Write-Host "未选择任何平台" -F Red;exit 1}
