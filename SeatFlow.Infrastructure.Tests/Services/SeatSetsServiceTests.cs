...
public class SeatSetsServiceTests : IDisposable
{
    ...
    private async Task CreateMockDataAsync()
    {
        ...
        var venueFile = new
        {
            version = "1.1",
            venueId = "venue-001",
            layout = new { id = "venue-001", name = "Test Venue", layoutType = 0, seats = new object[0], obstacles = new object[0] },
            contentHash = "abc123"
        };
    ...
    public async Task ExportAndImport_RoundTrip_PreservesAllData()
    {
        ...
        {
            IncludeAppSettings = true, IncludeVenues = true, IncludeRosters = true,
            IncludeSnapshots = false, IncludeStrategyConfig = true
        };
        var exportPath = Path.Combine(_tempRoot, "test.seatsets");

        // Act: Export
        var exportedCount = await service.ExportAsync(exportPath, selection);
        ...
        var result = await service.ImportAsync(exportPath, importSelection);
    ...
    public async Task Validate_ValidFile_ReturnsValid()
    {
        ...
        {
            IncludeAppSettings = true, IncludeVenues = true
        });

        // Act
        var result = await service.ValidateAsync(exportPath);
    ...
    public async Task Validate_OversizedFile_Rejects()
    {
        ...
        var result = await service.ValidateAsync(Path.Combine(_tempRoot, "nonexistent.seatsets"));
    ...
    public async Task Validate_InvalidJson_ReturnsInvalid()
    {
        ...
        await File.WriteAllTextAsync(invalidPath, "this is not json");

        // Act
        var result = await service.ValidateAsync(invalidPath);
    ...
    public async Task Export_PartialSelection_OnlyExportsSelected()
    {
        ...
        await service.ExportAsync(exportPath, selection);
    ...
    public async Task Export_EmptyDirectory_ReturnsZero()
    {
        ...
        // Act
        var count = await service.ExportAsync(exportPath, new SeatSetsExportSelection());
    ...
    public async Task ProbeCategories_CorrectlyIdentifiesAvailableCategories()
    {
        ...
        await service.ExportAsync(exportPath, new SeatSetsExportSelection
        {
            IncludeAppSettings = true, IncludeVenues = true, IncludeRosters = false,
            IncludeSnapshots = false, IncludeStrategyConfig = false
        });

        // Act
        var categories = await service.ProbeCategoriesAsync(exportPath);
    ...
    public async Task Import_PartialRestore_SkipsMissingCategories()
    {
        ...
        {
            IncludeAppSettings = true, IncludeVenues = true,
            IncludeRosters = true, IncludeStrategyConfig = true
        });
        ...
        {
            IncludeAppSettings = true, IncludeVenues = false,
            IncludeRosters = false, IncludeSnapshots = false, IncludeStrategyConfig = false
        };

        // Act
        var result = await service.ImportAsync(exportPath, partialSelection);
    ...
    public async Task Import_CreatesTargetDirectories()
    {
        ...
        {
            IncludeAppSettings = true, IncludeVenues = true, IncludeRosters = true,
            IncludeSnapshots = true, IncludeStrategyConfig = true
        });
